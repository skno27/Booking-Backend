...
export const paymentIntent: RequestHandler = async (req, res) => {
  ...
  let total: Decimal = new Decimal(transaction.orderTotal);
  ...
  try {
    ...
    await Promise.all([
      communication.sendEmail(
        ...
        "Booking Confirmation from Skeno",
        `Your mixing order has been confirmed! Your order confirmation is: ${confirmationString}
