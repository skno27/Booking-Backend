...
import { File, Mix, MixType, FileType } from "@prisma/client";
  ...
  try {
    const result = await prisma.$transaction(async (prisma) => {
      const transaction = await prisma.transaction.create({
        data: {
          customer_id: customer as number,
        },
      });
      ...
      const mixPromises = mixData.map(
        (slice: { songName: string; type: MixType; print: FileType }) => {
      ...
      const filePromises = fileData.map(
        (slice: { songName: string; print: FileType }) => {
      ...
      const generatePrice = (mixArray: Mix[], fileArray: File[]) => {
